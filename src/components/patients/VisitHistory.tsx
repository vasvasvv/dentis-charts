...
import { Plus, Calendar as CalendarIcon, Trash2, Clock, ChevronDown, ChevronUp, Edit2 } from 'lucide-react';
...
export function VisitHistory({ patientId }: VisitHistoryProps) {
  ...
  // Безпечне сортування — перевіряємо що date існує
  ...
  const editingVisitData = editingVisit
    ? patient.visits.find(v => v.id === editingVisit)
    : null;

  return (
    <div className="border-t mt-4 pt-4">
      {/* ── Заголовок з кнопкою акордеону ── */}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 animate-fade-in">
          ...
          <div className="flex justify-center">
            <Calendar
              ...
              modifiers={{ future: futureDates, past: pastDates }}
              modifiersClassNames={{
      ...
      {/* Модалка редагування візиту */}
      {editingVisitData && (
        <VisitModal
          isOpen={editingVisit !== null}
          onClose={() => setEditingVisit(null)}
          patientId={patientId}
          visitId={editingVisit || undefined}
          initialDate={editingVisitData.date}
          initialType={editingVisitData.type}
          initialNotes={editingVisitData.notes}
          initialDoctorId={editingVisitData.doctorId}
        />
      )}
