...
export function ClinicProvider({ children }: { children: React.ReactNode }) {
    ...
    const getDoctors = useCallback(async () => {
        ...
        try {
            ...
            if (res.ok) {
                ...
                setDoctors(mapped);
                // Встановити лікаря за замовчуванням тільки при першому завантаженні
                if (!doctorsLoadedRef.current) {
                    doctorsLoadedRef.current = true;
                    const isDoctor = currentUser?.role === 'doctor';
                    if (isDoctor && currentUser?.name) {
                        // Знайти лікаря зі списку, чиє ім’я збігається з поточним юзером
                        const matchedDoctor = mapped.find(
                            (d: any) => d.name.toLowerCase().trim() === currentUser.name.toLowerCase().trim()
                        );
                        setSelectedDoctorId(matchedDoctor ? matchedDoctor.id : (mapped[0]?.id ?? null));
                    } else {
                        // Не лікар (адмін, super-admin) — показати всіх
    ...
    }, [token, currentUser]); // ← додано currentUser для визначення дефолтного лікаря
    ...
    useEffect(() => {
        if (token) {
            ...
            // При logout скидаємо ref, щоб наступний логін правильно встановив лікаря
