...
// Кешуємо поза компонентом щоб не перезавантажувати щоразу
...
export function useBase64Images() {
  ...
  useEffect(() => {
    ...
    Promise.all(IMAGE_URLS.map(toBase64)).then(result => {
      cachedImages = result;
      setImages(result);
    });
  }, []);

  return images; // null поки завантажується, потім масив з 6 base64 рядків
}